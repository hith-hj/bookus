<template>
 ...
 <table class="table table-bordered">
  ...
  <tbody>
    <tr  v-for="itema in row.item.appointment_services"
        :key="itema.id">
...
export default {
  ...
  data() {
    return {
      fields: ['center','client', 'booking_date','total','show_details', 'action'],
      services1:[],
      users: {},
      userSearch: null,
      perPage: 10,
  ...
  methods: {
    getUsers(page, perPage) {
      ...
      axios
        .get(`/api/admin/appointments?page=${page}&limit=${perPage}`, config)
        .then(res => {
          this.users = res.data.content
          this.services1 =this.users.appointment_services

          console.log( "services 1",this.services1);
    ...
    getCategoriesFilter() {
      let url = `/api/admin/appointments?page=${
        this.currentPage
      }&limit=${
        this.perPage}`
      ...
      axios
        ...
        .then(res => {
          this.users = res.data.content
          this.services1 =this.users.appointment_services
          console.log( "services2",this.services1);
