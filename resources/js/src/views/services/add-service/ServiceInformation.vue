...
export default {
  ...
  methods: {
    validationForm() {
      this.$refs.simpleRules.validate().then(success => {
        if (success) {
          ...
          if(this.checkedBookinOnline)
          formdata.append('online_booking', 1)
            else
          formdata.append('online_booking', 0)
          ...
          formdata.append('price_type', this.PriceType)
            if(this.extraTime)
          formdata.append('extra_time', 1)
            else
                formdata.append('extra_time', 0)


          axios.post('/api/admin/services', formdata, config)
            .then(res => {
              this.$router.replace('/servicesList').then(() => {
                this.$toast({
    ...
    getCategor() {
      ...
      axios
        .get('/api/admin/get_all_categories', config)
        .then(res => {
