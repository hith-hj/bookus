<template>
  <div>
    ...
    <form-wizard color="#7367F0" :title="null" :subtitle="null" layout="vertical" finish-button-text="Submit"
      ...
      <!-- chose services -->
      <tab-content title="Center Services"  :before-change="validateAsync">
        <div v-if="loading" class="w-50 m-auto">
      ...
      <tab-content title="TEAM">
        <b-row>
          ...
          <b-col md="6">
            <b-form-group label-for="v-language" label="Shift">
              <v-select id="Language" v-model="shift" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
           :options="shifts"
                :disabled="!disibleShift"  label="text" />
            </b-form-group>
          </b-col>
          <b-col md="6">
            <div>
...
SuccessMsg,
  ...
  watch: {
    selectDate() {
      if (this.selectDate != '') this.disibleShift = true
  ...
  methods: {
    ...
    getCategories() {
      ...
      axios
        .get(`/api/admin/get-center-categories-admin/${this.$route.params.id}`, config)
        .then(res => {
